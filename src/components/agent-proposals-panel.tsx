...
export function AgentProposalsPanel({ villages }: AgentProposalsPanelProps) {
  ...
  const getEffectiveSelectedCandidateId = (proposal: LatestProposal) => {
    if (!proposal) {
      return null;
    }

    return (
      selectedCandidates[proposal.id] ??
      proposal.selectedCandidateId ??
      proposal.recommendedCandidateId ??
  ...
  const actOnProposal = (
    ...
    startTransition(async () => {
      const response = await fetch(`/api/agent-proposals/${proposalId}`, {
        ...
        body: JSON.stringify({
          action,
          candidateId: action === "approve" ? selectedCandidateId : undefined,
        }),
      });
    ...
    <section className="grid gap-4">
      ...
      <div className="grid gap-4">
        {villages.map((village) => {
          ...
          const recommended = proposal?.candidates.find((candidate) => candidate.id === proposal.recommendedCandidateId) ?? proposal?.candidates[0] ?? null;
          const isBusy = pendingProposalId === proposal?.id;
          const selectedCandidateId = getEffectiveSelectedCandidateId(proposal);
                  ...
                  <>
                        ...
                        <button
                          ...
                          className={
                            candidate.id === selectedCandidateId
                              ? "rounded-2xl border border-amber-300/35 bg-amber-300/10 p-4 text-left shadow-[0_0_0_1px_rgba(252,211,77,0.12)]"
                              : "rounded-2xl border border-white/8 bg-white/5 p-4 text-left hover:border-white/15 hover:bg-white/8"
                          }
                          onClick={() =>
                        ...
                        >
                          <div className="flex items-start justify-between gap-3">
                            <div>
                              <div className="flex flex-wrap items-center gap-2">
                                ...
                                {candidate.id === proposal.recommendedCandidateId ? (
                                  <Badge className="bg-amber-300/15 text-amber-200 hover:bg-amber-300/15">
                          ...
                          <div className="mt-3 flex flex-wrap gap-2 text-sm text-stone-200">
                            ...
                            <Badge className="bg-black/15 text-stone-200 hover:bg-black/15">
                              Heurística {candidate.heuristicScore.toFixed(0)}
                            </Badge>
                            <Badge className="bg-black/15 text-stone-200 hover:bg-black/15">
                              Aprendido {candidate.learnedScore.toFixed(1)}
                            </Badge>
                            {candidate.timeToAffordText ? (
                              <Badge className="bg-black/15 text-stone-200 hover:bg-black/15">
                                ETA {candidate.timeToAffordText}
                              </Badge>
                            ) : null}
                          </div>
                        </button>
                    ...
                    <div className="flex flex-wrap gap-2">
                      <Button
                        ...
                        onClick={() =>
                          actOnProposal(proposal.id, "approve", selectedCandidateId)
                        }
                        type="button"
                      >
                      ...
                      <Button
                        ...
                        onClick={() => actOnProposal(proposal.id, "reject", null)}
                        type="button"
                        variant="outline"
                      >
                        <X />
